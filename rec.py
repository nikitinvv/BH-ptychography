...
class Rec:
    ...
    def BH(self, data, vars, pars):
        ...
        for i in range(pars["niter"]):
            ...
            t = time.time()
            self.calc_phi(reused, data)
            print('calc_phi',time.time()-t)

            # debug and visualization
            t = time.time()
            self.error_debug(vars, pars, reused, data, i)
            print('error_debug',time.time()-t)

            self.vis_debug(vars, pars, i)
            ...
            if i == 0 or pars["method"] == "BH-GD":
                ...
                # conjugate direction
                t = time.time()
                beta = self.calc_beta(vars, grads, etas, reused, data)
            ...
            # step length
            t = time.time()
            alpha, top, bottom = self.calc_alpha(vars, grads, etas, reused, data)
            print('alpha',time.time()-t)
            # debug approxmation
            self.plot_debug(vars, etas, pars, top, bottom, alpha, data, i)
    ...
    def gradients(self, vars, reused):
        ...
        dprb = self.gradient_q(spsi, big_phi)
    ...
    def plot_debug(self, vars, etas, pars, top, bottom, alpha, d, i):
    ...
    def vis_debug(self, vars, pars, i):
    ...
    def error_debug(self,vars, pars, reused, d, i):
        """Visualization and data saving"""
