...
class Rec:
    ...
    def gradient_psi(self, scode, gradF, q):
        # memory for result
        flg = chunking_flg(locals().values())        
        if flg:
            psi = []
        ...
        @gpu_batch(self.nchunk, self.ngpus, axis_out=0, axis_inp=0)                
        def _gradient_psi(self, psi, scode, gradF, q):            
            psi[:] += cp.sum(cp.conj(self.Dc(scode*q)) * gradF,axis=0)
        _gradient_psi(self, psi, scode, gradF, q)
        if flg:
    ...
    def gradients(self, vars, reused):
        ...
        dpsi = self.gradient_psi(scode, gradF, q)
